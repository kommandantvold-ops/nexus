...
export async function GET(req: NextRequest) {
  ...
  const { data: agent } = await db
    .from('colony_agents')
    .select('bee_id, position_q, position_r, status')
  ...
  // Get bee name
  const { data: bee } = await db.from('bees').select('name').eq('id', agent.bee_id).single()

  // Fetch world state
  const [hexes, agents, resources, chat, actions] = await Promise.all([
    ...
    db.from('colony_actions').select('bee_id, action_type, details, created_at').order('created_at', { ascending: false }).limit(20),
  ])

  // Get bee names for agents
  const beeIds = [...new Set([...(agents.data || []).map(a => a.bee_id), ...(chat.data || []).map(c => c.bee_id)])]
  const { data: bees } = await db.from('bees').select('id, name').in('id', beeIds)
  ...
  return NextResponse.json({
    you: {
      bee_id: agent.bee_id,
      name: bee?.name || agent.bee_id,
      position: { q: agent.position_q, r: agent.position_r }
    },
